...
function inspect(value, { depth = 2, hidden = false } = {}) {
  ...
  if (is.promise(value))
    return Promise.race([value, UMP]).then(
      v => `Promise { <${v === UMP ? 'pending' : 'fulfilled'}> }`,
      () => 'Promise { <rejected> }'
    )
...
function formatValue(ctx, value, recurseTimes) {
  if (is.multi(['digit', 'string', 'boolean', 'nullish'], value)) return inspect(value)
  ...
  if (is.error(value) && (keys.indexOf('message') >= 0 || keys.indexOf('description') >= 0)) {
